export type MegaMenuLink = { label: string; href: string; description: string; key?: string };
export type MegaMenuCategory = { label: string; columns: MegaMenuLink[][] };

// `href` is the fallback/default; entries with a `key` resolve their live URL from
